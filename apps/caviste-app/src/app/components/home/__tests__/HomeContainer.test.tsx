  ...
  return function MockHomeView(props: MockHomeViewProps) {
    return (
      <div data-testid="home-view">
        <div data-testid="collections-loading">
          {props.collectionsLoading.toString()}
        </div>
        <div data-testid="collections-error">
        ...
        <button onClick={props.onItemClick} data-testid="item-click">
          Item Click
        </button>
        <button onClick={props.onLoadMore} data-testid="load-more">
          Load More
        </button>
        <button onClick={props.onClosePopup} data-testid="close-popup">
          Close Popup
        </button>
      </div>
    );
...
describe("HomeContainer", () => {
  const mockCollectionsData = {
    collections: collectionsFactory({ length: 2 }),
  ...
  it("renders HomeView with correct props", () => {
    ...
    expect(screen.getByTestId("collections-loading")).toHaveTextContent(
      "false",
    );
  ...
  it("sets document title on mount", () => {
    ...
    expect(document.title).toBe(
      "Edouard Caviste personnel - Vins propres de vignerons",
    );
  });
  ...
  it("passes error state to HomeView", () => {
    ...
    expect(screen.getByTestId("collections-error")).toHaveTextContent(
      errorMessage,
    );
  });
  ...
  it("passes popup state to HomeView", () => {
    mockUseCollections.mockReturnValue({
      ...
      popupProducts: [productFactory(), productFactory()],
      popupLoading: true,
      hasNextPage: true,
    });
