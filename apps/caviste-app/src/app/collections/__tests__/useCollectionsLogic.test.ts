...
describe("useCollectionsLogic", () => {
  const defaultProps = {
    initialCollections: collectionsFactory({ length: 15 }),
    searchParams: {},
  };
  ...
  it("should sort collections array by name in descending order", () => {
    ...
    const lastCollection =
      result.current.collections[result.current.collections.length - 1];

    expect(
      firstCollection.title.localeCompare(lastCollection.title)
    ).toBeGreaterThan(0);
  });
  ...
  it("should handle collection item click and call openCollection", async () => {
    ...
    await act(async () => {
      await result.current.onItemClick("collection-1", "Test Collection", [
        "tag1",
      ]);
    });
