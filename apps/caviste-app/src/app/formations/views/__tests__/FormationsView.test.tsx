...
describe("FormationsView", () => {
  ...
  it("displays formation content and form", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Formations en Œnologie", level: 1 }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Nos Formations", level: 2 }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Demande d'Information", level: 2 }),
    ).toBeInTheDocument();
  ...
  it("renders accessible form with proper labels", () => {
    ...
    expect(
      screen.getByRole("textbox", { name: "Nom complet *" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("textbox", { name: "Email *" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("textbox", { name: "Message (optionnel)" }),
    ).toBeInTheDocument();
    expect(
  ...
  it("has a form with submit handler", () => {
    ...
    const form = screen
      .getByRole("button", { name: "Envoyer ma demande" })
      .closest("form");
  ...
  it("has proper ARIA attributes for accessibility", () => {
    ...
    expect(screen.getByText("Votre nom complet est requis")).toHaveClass(
      "sr-only",
    );
    expect(screen.getByText("Votre adresse email est requise")).toHaveClass(
      "sr-only",
    );
  });
