...
describe("CartItem", () => {
  ...
  describe("Rendering", () => {
    it("displays item information correctly", () => {
      ...
      expect(screen.getByText(`${item.unitPrice} ${item.currency} l'unité`)).toBeInTheDocument();
  ...
  describe("User Interactions", () => {
    ...
    it("calls onQuantityChange when quantity is decreased", async () => {
      const user = userEvent.setup();
      const [item] = cartLinesFactory({ overrideCartLines: [{ quantity: 3 }] });
      renderCartItem({ item });
  ...
  describe("Stock Management", () => {
    it("shows stock warning when quantity available is low", () => {
      const [item] = cartLinesFactory({ overrideCartLines: [{ quantityAvailable: 5 }] });
      renderCartItem({ item });
