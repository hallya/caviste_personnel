  ...
  return (
    <article className="flex items-stretch justify-between border-b border-neutral-200 pb-4">
      <div className="flex items-stretch space-x-2 md:space-x-4 flex-1 min-w-0">
        ...
        <div className="flex-1 flex flex-col justify-between min-w-0">
          <header className="min-w-0">
            <h2 className="text-subtitle text-primary-600 leading-tight">
              {item.title}
            </h2>
      ...
      <div className="text-right ml-2 md:ml-4 flex flex-col justify-center flex-shrink-0">
        <p
          className="text-subtitle text-primary-600 font-medium"
          aria-label={`Prix total pour ${item.title}: ${item.lineTotal}`}
        >
          {item.lineTotal}
        </p>
        <p className="text-caption text-neutral-500 mt-1">
          {`${item.unitPrice} ${item.currency} l'unité`}
        </p>
      </div>
    </article>
  );
