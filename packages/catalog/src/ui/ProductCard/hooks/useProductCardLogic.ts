...
export function useProductCardLogic({ product }: UseProductCardLogicProps) {
  ...
  const availableQuantity = useMemo(
    () => product.totalInventory - cartQuantity,
    [product.totalInventory, cartQuantity]
  );
  ...
  const handleAddToCart = useCallback(
    async (quantity: number, mode: "bottle" | "carton") => {
      const isAdding =
      ...
      if (!canAdd) {
        showCartNotification(
          "error",
          "Stock insuffisant",
          `Stock insuffisant pour ajouter ${quantity} ${modeText} de ${product.title}`,
          {
            autoClose: false,
          }
        );
      ...
      showCartNotification(
        "loading",
        "En cours d'ajout...",
        `${quantity} ${modeText} de ${product.title}`,
        {
          autoClose: false,
          id: loadingId,
        }
      ...
      try {
        ...
        if (result) {
          ...
          showCartNotification(
            "success",
            "Produit ajouté",
            `${quantity} ${modeText} de ${product.title} ${addedText} au panier`,
            {
              autoClose: true,
              replaceId: loadingId,
            }
