...
export function createShopifyClient(
  ...
  if (!finalConfig.url) {
    throw new Error(
      "NEXT_PUBLIC_SHOPIFY_STORE_DOMAIN environment variable is not defined",
    );
  }
  if (!finalConfig.token) {
    throw new Error(
      "NEXT_PUBLIC_SHOPIFY_STOREFRONT_ACCESS_TOKEN environment variable is not defined",
    );
  }
  if (!finalConfig.apiVersion) {
    throw new Error(
      "NEXT_PUBLIC_SHOPIFY_API_VERSION environment variable is not defined",
    );
  }
