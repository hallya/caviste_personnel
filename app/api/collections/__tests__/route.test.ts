...
import { IDS, HANDLES, DATA, URLS } from "../../../__tests__/constants";
import {
  CollectionFactories,
...
describe("/api/collections", () => {
  ...
  describe("Successful responses", () => {
    it("should fetch and transform collections successfully", async () => {
      ...
      const mockCollectionsResponse =
        createMockCollectionsResponse(collections);
    ...
    it("should handle malformed collection tags gracefully", async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "Failed to parse collection tags for collection-malformed"
        ),
        expect.any(Error)
      );
  ...
  describe("Error handling", () => {
    ...
    it("should handle GraphQL errors", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "GraphQL errors:",
        expect.stringContaining(
          '"Field \\"collections\\" doesn\'t exist on type \\"QueryRoot\\""'
        )
      );
  ...
  describe("Video metafield handling", () => {
    it("should handle GenericFile video metafield", async () => {
      ...
      expect(data.collections[0].videoCollection).toBe(
        "https://example.com/generic-video.mp4"
      );
    });
    ...
    it("should handle Video metafield with sources", async () => {
      ...
      expect(data.collections[0].videoCollection).toBe(
        "https://example.com/video-source.mp4"
      );
    });
