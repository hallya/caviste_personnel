...
export function useCollectionsLogic({ initialCollections, searchParams }: UseCollectionsLogicProps) {
  ...
  const updateURL = useCallback((search: string, sort: string, order: string) => {
  ...
  const handleItemClick = async (handle: string, title?: string, collectionTags?: string[]) => {
    ...
    await openCollection(handle, collectionTitle, tags);
  };
