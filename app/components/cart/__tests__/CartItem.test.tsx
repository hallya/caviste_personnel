...
describe('CartItem', () => {
  ...
  describe('Rendering', () => {
    it('displays item information correctly', () => {
      ...
      expect(screen.getByRole('spinbutton', { name: /quantité/i })).toHaveValue(item.quantity);
    });
  ...
  describe('User Interactions', () => {
    it('calls onQuantityChange when quantity is increased', async () => {
      ...
      expect(mockOnQuantityChange).toHaveBeenCalledWith(item.id, item.quantity + 1);
    });
  ...
  describe('Stock Management', () => {
    ...
    it('shows out of stock message when not available for sale', () => {
      const item = CartTestFactories.createCartLine({ availableForSale: false });
      renderCartItem({ item });
  ...
  describe('Accessibility', () => {
    it('has proper ARIA labels for interactive elements', () => {
      ...
      expect(removeButton).toHaveAttribute('aria-label', expect.stringContaining(item.title));
    ...
    it('has proper ARIA label for total price', () => {
      ...
      expect(total).toHaveAttribute('aria-label', expect.stringContaining('Prix total'));
    });
    ...
    it('has semantic structure with proper roles', () => {
      ...
      expect(screen.getByRole('article')).toBeInTheDocument();
      expect(screen.getByRole('banner')).toBeInTheDocument();
      expect(screen.getByRole('group', { name: /sélecteur de quantité/i })).toBeInTheDocument();
    });
    ...
    it('has proper navigation structure', () => {
      ...
      const itemHeading = screen.getByRole('heading', { level: 2, name: item.title });
