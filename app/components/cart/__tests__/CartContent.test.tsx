...
describe('CartContent Accessibility', () => {
  ...
  const renderCartContent = (overrides = {}) => {
    const defaultProps = {
      cart: CartTestFactories.createCart(),
      actionLoading: false,
  ...
  it('has proper ARIA attributes for total', () => {
    ...
    const total = screen.getByText('300.00 EUR', { selector: 'span[aria-live="polite"]' });
  ...
  it('has proper semantic structure', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Votre panier' })).toBeInTheDocument();
    expect(screen.getByRole('group', { name: /actions du panier/i })).toBeInTheDocument();
  });
  ...
  it('has proper button labels and descriptions', () => {
    ...
    const removeButton = screen.getByRole('button', { name: /supprimer.*château margaux/i });
    const checkoutButton = screen.getByRole('button', { name: /finaliser.*commande/i });
    const continueButton = screen.getByRole('link', { name: /continuer.*achats/i });

    expect(removeButton).toHaveAttribute('aria-label', expect.stringContaining('Supprimer'));
    expect(checkoutButton).toHaveAttribute('aria-label', expect.stringContaining('Finaliser'));
    expect(continueButton).toHaveAttribute('aria-label', expect.stringContaining('Continuer'));
  });
  ...
  it('has proper error handling accessibility', () => {
    const errorCart = CartTestFactories.createCart({
      lines: [
        CartTestFactories.createCartLine({ availableForSale: false })
      ]
    });
  ...
  it('has proper navigation structure', () => {
    ...
    const mainHeading = screen.getByRole('heading', { level: 1, name: 'Votre panier' });
    const itemHeading = screen.getByRole('heading', { level: 2, name: 'Château Margaux 2018' });
  ...
  it('has proper group semantics', () => {
    ...
    const quantityGroup = screen.getByRole('group', { name: /sélecteur de quantité/i });
    const actionsGroup = screen.getByRole('group', { name: /actions du panier/i });
