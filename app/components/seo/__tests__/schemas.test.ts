import { ORGANIZATION_SCHEMA, EDUCATIONAL_ORGANIZATION_SCHEMA, WEBSITE_SCHEMA } from '../schemas';
import { SOCIAL_LINKS } from '../../home/views/constants';
...
describe('JSON-LD Schemas', () => {
  describe('ORGANIZATION_SCHEMA', () => {
    it('has correct structure and required fields', () => {
      ...
      expect(ORGANIZATION_SCHEMA.url).toBe('https://caviste-personnel.vercel.app');
    ...
    it('has valid contact information', () => {
      ...
      expect(ORGANIZATION_SCHEMA.contactPoint.email).toBe('edouard.dulac@gmail.com');
  ...
  describe('EDUCATIONAL_ORGANIZATION_SCHEMA', () => {
    it('has correct structure and required fields', () => {
      expect(EDUCATIONAL_ORGANIZATION_SCHEMA['@context']).toBe('https://schema.org');
      expect(EDUCATIONAL_ORGANIZATION_SCHEMA['@type']).toBe('EducationalOrganization');
      expect(EDUCATIONAL_ORGANIZATION_SCHEMA.hasOfferCatalog).toBeDefined();
      expect(EDUCATIONAL_ORGANIZATION_SCHEMA.hasOfferCatalog['@type']).toBe('OfferCatalog');
    });

    it('has three course offerings', () => {
      const offerings = EDUCATIONAL_ORGANIZATION_SCHEMA.hasOfferCatalog.itemListElement;
    ...
    it('has valid course structure', () => {
      const firstCourse = EDUCATIONAL_ORGANIZATION_SCHEMA.hasOfferCatalog.itemListElement[0];
  ...
  describe('WEBSITE_SCHEMA', () => {
    ...
    it('has valid search action', () => {
      ...
      expect(WEBSITE_SCHEMA.potentialAction['query-input']).toBe('required name=search_term_string');
    });
  });
  ...
  describe('Schema validation', () => {
    it('all schemas are valid JSON', () => {
      ...
      expect(() => JSON.stringify(EDUCATIONAL_ORGANIZATION_SCHEMA)).not.toThrow();
      expect(() => JSON.stringify(WEBSITE_SCHEMA)).not.toThrow();
    });

    it('all schemas have required @context and @type', () => {
      const schemas = [ORGANIZATION_SCHEMA, EDUCATIONAL_ORGANIZATION_SCHEMA, WEBSITE_SCHEMA];
