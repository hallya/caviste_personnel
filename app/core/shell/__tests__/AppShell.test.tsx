...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <div data-testid="module-api">{moduleAPI ? "API Available" : "No API"}</div>
    </div>
  );
}
...
describe("AppShell", () => {
  ...
  it("registers modules correctly", async () => {
    ...
    const registerButton = screen.getByRole("button", { name: "Register Module" });
  ...
  it("returns null for non-existent modules", () => {
    function NonExistentModuleTest() {
      ...
      return (
        <div>
          <div data-testid="non-existent-loaded">{isModuleLoaded("non-existent").toString()}</div>
          <div data-testid="non-existent-api">{getModuleAPI("non-existent") ? "Found" : "Not Found"}</div>
          <div data-testid="non-existent-module">{getModule("non-existent") ? "Found" : "Not Found"}</div>
        </div>
      );
    }
    ...
    expect(screen.getByTestId("non-existent-loaded")).toHaveTextContent("false");
    expect(screen.getByTestId("non-existent-api")).toHaveTextContent("Not Found");
    expect(screen.getByTestId("non-existent-module")).toHaveTextContent("Not Found");
  });
